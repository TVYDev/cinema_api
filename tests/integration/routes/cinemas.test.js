...
describe('Cinemas', () => {
  ...
  describe('GET /api/v1/cinemas', () => {
    it('should return 200, and return all cinemas', async () => {
      ...
      expect(items.some((c) => c.name === 'Delee Cinma Takmao')).toBeTruthy();

      expect(
        items.some((c) => c.address === 'Toul Kork, Phnom Penh, Cambodia')
      ).toBeTruthy();
      expect(items.some((c) => c.address === 'Takhmao, Cambodia')).toBeTruthy();

      expect(items.some((c) => c.openingHours === '7AM - 10PM')).toBeTruthy();
      expect(items.some((c) => c.openingHours === '8AM - 9PM')).toBeTruthy();
  ...
  describe('POST /api/v1/cinemas', () => {
    ...
    it('should return 201, and return cinema and status 200 if operation is successful', async () => {
      ...
      expect(res.body.data).toHaveProperty('name', 'Delee Cinma Phnom Penh');
  ...
  describe('GET /api/v1/cinemas/:id', () => {
    ...
    it('should return 200, and return a cinema if valid id is passed', async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.data).toHaveProperty('_id', cinema._id.toHexString());
      expect(res.body.data).toHaveProperty('name', 'Delee Cinma Phnom Penh');
      expect(res.body.data).toHaveProperty(
        'address',
        'Toul Kork, Phnom Penh, Cambodia'
  ...
  describe('PUT /api/v1/cinemas/:id', () => {
    ...
    it('should return 200, and return the updated cinema if request is valid', async () => {
      ...
      expect(res.body.data).toHaveProperty('address', 'Takhmao, Cambodia');
  ...
  describe('DELETE /api/v1/cinemas/:id', () => {
    ...
    const exec = () => request(server).delete(`/api/v1/cinemas/${cinemaId}`);
  ...
  describe('PUT /api/v1/cinemas/:id/photo', () => {
    ...
    const exec = () => request(server).put(`/api/v1/cinemas/${cinemaId}/photo`);
  ...
  describe('PUT /api/v1/cinemas/:id/layout-image', () => {
    ...
    it('should return 400 if uploaded file is not an image fil', async () => {
      const res = await exec().attach('file', textFileUrl);
